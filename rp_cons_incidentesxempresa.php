...
if (bd_verificar_privilegios('rp_cons_incidentesxempresa.php',$_SESSION['usuario']['nivel_id'])!='CONCEDER')
{
...
$reporte = sql2array("SELECT id,fecha,hora,personal_id,empresa_id,area,seccion_id,proyecto,clase_id,tipo_id,condicion_id,accion_id,region_id,causa_id,
							descripcion,tiempo_perdido,personal_reg_id,acto FROM incidentes WHERE empresa_id = '$id'");
...
$empresa_id = bd_obt_empresas(0,$_REQUEST['id']);
$smarty->assign('datos',$reporte);
$smarty->assign('empresa_id',$empresa_id);
$smarty->disp();
