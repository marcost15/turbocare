...
if (bd_verificar_privilegios('rp_frm_morbilidadxfecha.php',$_SESSION['usuario']['nivel_id'])!='CONCEDER')
{
	ir('negacion_usuario.php');
}

$f1=new dbFormHandler('morbilidadxfecha',NULL,'onclick="highlight(event)"');
$f1->setLanguage('es');
...
function procesar($d)
{
	...
	if ($resp == 1)
	{
		...
		return false;
		ir("rp_frm_morbilidadxfecha.php");
	}
	else
	{
		$fecha_ini = $d['fecha_ini'];
		$fecha_fin = $d['fecha_fin'];
		ir("rp_cons_morbilidadxfecha.php?fecha_ini=$fecha_ini&fecha_fin=$fecha_fin");
	}
}
